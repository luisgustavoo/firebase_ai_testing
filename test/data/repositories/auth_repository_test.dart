...
void main() {
  group('AuthRepository Unit Tests', () {
    ...
    group('getProfile', () {
      ...
      test('should handle expired token (401)', () async {
        const expiredToken = 'expired_token';
        await tokenStorage.saveToken(expiredToken);

        final mockClient = MockClient((request) async {
          return http.Response(
            json.encode({'error': 'Token expired'}),
            401,
          );
        });
        ...
        apiService = ApiService(tokenStorage, mockClient);
        await apiService.init();
        apiService.authToken = expiredToken;
        authRepository = AuthRepository(apiService, tokenStorage);

        // Verify token exists before call
